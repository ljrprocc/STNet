...
class GateTransposedConv2d(nn.ConvTranspose2d):
    def __init__(self, activation=None, *args, **kwargs):
        super(GateTransposedConv2d, self).__init__(*args, **kwargs)
        self.activation = activation
...
class GatedCoarse2FineModel(nn.Module):
    def __init__(self, hidden_channels=24, dilation_depth=4):
    ...
    def build_inpaint_model(self):
        self.conv1s = []
        self.bn1s = []
        self.conv1s.append(GateConv2d(self.gen_relu, 3, self.hidden_channels * 2, 5, 2, padding=2))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels))
        self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels, self.hidden_channels * 2, 3, 1, padding=1))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels))
        self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels, self.hidden_channels * 4, 3, 1, padding=1))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels * 2))
        self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels * 2, self.hidden_channels * 4, 3, 2, padding=1))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels * 2))
        self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels * 2, self.hidden_channels * 8, 3, 1, padding=1))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels * 4))
        self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels * 4, self.hidden_channels * 8, 3, 1, padding=1))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels * 4))
        for i in range(self.dilation_depth):
            self.conv1s.append(GateConv2d(self.gen_relu, self.hidden_channels*4, self.hidden_channels*8, 3, 1, padding=2**(i+1), dilation=2**(i+1)))
            self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels*4))
        self.conv1s = nn.ModuleList(self.conv1s)
        self.bn1s = nn.ModuleList(self.bn1s)
        self.conv2s = []
        self.bn2s = []
        self.conv2s.append(GateConv2d(self.gen_relu, 3, self.hidden_channels * 2, 5, 2, padding=2))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels))
        self.conv2s.append(GateConv2d(self.gen_relu, self.hidden_channels, self.hidden_channels * 2, 3, 1, padding=1))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels))
        self.conv2s.append(GateConv2d(self.gen_relu, self.hidden_channels, self.hidden_channels * 4, 3, 1, padding=1))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels * 2))
        self.conv2s.append(GateConv2d(self.gen_relu, self.hidden_channels * 2, self.hidden_channels * 4, 3, 2, padding=1))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels * 2))
        self.conv2s.append(GateConv2d(self.gen_relu, self.hidden_channels * 2, self.hidden_channels * 8, 3, 1, padding=1))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels * 4))
        self.conv2s.append(GateConv2d(self.gen_relu, self.hidden_channels * 4, self.hidden_channels * 8, 3, 1, padding=1))
        self.bn2s.append(nn.InstanceNorm2d(self.hidden_channels * 4))
        self.conv2s.append(ContextualAttention(ksize=3, stride=1, rate=2, use_cuda=True))
        self.conv2s = nn.ModuleList(self.conv2s)
        self.bn2s = nn.ModuleList(self.bn2s)
        self.total_conv = []
        self.total_bn = []
        self.total_conv.append(GateConv2d(self.gen_relu, self.hidden_channels*8, self.hidden_channels*8, 3, 1, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.gen_relu, self.hidden_channels*4))
        self.total_conv.append(GateConv2d(self.gen_relu, self.hidden_channels*4, self.hidden_channels*8, 3, 1, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels*2))
        self.total_conv.append(GateTransposedConv2d(self.gen_relu, self.hidden_channels*4, self.hidden_channels*4
        , 3, 2, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels*2))
        self.total_conv.append(GateConv2d(self.gen_relu, self.hidden_channels*2, self.hidden_channels*4, 3, 1, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels*2))
        self.total_conv.append(GateTransposedConv2d(self.gen_relu, self.hidden_channels*2, self.hidden_channels*4, 3, 2, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels*2))
        self.total_conv.append(GateConv2d(self.gen_relu, self.hidden_channels*2, self.hidden_channels*2, 3, 1, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels))
        self.total_conv.append(GateConv2d(self.gen_relu, self.hidden_channels, self.hidden_channels, 3, 1, padding=1))
        self.total_bn.append(nn.InstanceNorm2d(self.hidden_channels // 2))
...
class TinyCoarse2FineModel(nn.Module):
    ...
    def build_inpaint_model(self):
        self.conv1s = []
        self.bn1s = []
        self.conv1s.append(nn.Conv2d(3, self.hidden_channels, 5, 2, padding=2))
        self.bn1s.append(nn.InstanceNorm2d(self.hidden_channels))
