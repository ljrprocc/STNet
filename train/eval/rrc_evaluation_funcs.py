...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    ...
    for name in archive.namelist():
        ...
        keyName = name
        if fileNameRegExp!="":
            # print(re.match(fileNameRegExp, name))
            m = re.match(fileNameRegExp,name.split('/')[-1])
            if m is None:
                addFile = False
            else:
                if len(m.groups())>0:
...
def validate_clockwise_points(points):
    ...
    point = [
                [eval(points[0]) , eval(points[1])],
                [eval(points[2]) , eval(points[3])],
                [eval(points[4]) , eval(points[5])],
                [eval(points[6]) , eval(points[7])]
            ]
    edge = [
